...
class MetadataStore:
    """SQLite-backed store for projects, frames, and messages."""
        ...
        with self._connect() as conn:
            conn.execute(
                """INSERT INTO projects (id, name, description, context, created_at, updated_at)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (project_id, name, description, context, now, now),
            )
        return Project(
            id=project_id,
            name=name,
    ...
    def create_frame(
        ...
        *,
        ...
        now = _now_ms()
        frame_id = _new_id()
        root = root_frame_id or frame_id
        with self._connect() as conn:
            conn.execute(
                """INSERT INTO frames (id, project_id, parent_frame_id, root_frame_id, name,
                   status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'active', ?, ?)""",
                (frame_id, project_id, parent_frame_id, root, name, now, now),
            )
        return Frame(
            id=frame_id,
    ...
    def append_message(self, *, frame_id: str, role: str, content: Any) -> Message:
        now = _now_ms()
        with self._connect() as conn:
            row = conn.execute(
            ...
            conn.execute(
                """INSERT INTO messages (frame_id, idx, role, content, created_at)
                   VALUES (?, ?, ?, ?, ?)""",
                (frame_id, idx, role, _serialize_content(content), now),
            )
    ...
    def list_messages(self, frame_id: str) -> list[Message]:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM messages WHERE frame_id = ? ORDER BY idx", (frame_id,)
            ).fetchall()
    ...
    def message_count(self, frame_id: str) -> int:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT COUNT(*) AS cnt FROM messages WHERE frame_id = ?", (frame_id,)
            ).fetchone()
